...
def main(result_folder, proportion_of_saturations, net_path, total_timesteps, model_path, num_of_episodes, seed, fix_seed, fixed_ts, learning_rate, learning_starts, train_freq, target_update_interval, exploration_initial_eps, exploration_final_eps, training_fraction, verbose, tripinfo, discount_factor, batch_size, replay_buffer_size):
    ...
    train = Train(result_folder, net_path, route_path, total_timesteps, model_path, num_of_episodes=num_of_episodes, seed=seed, fix_seed=fix_seed, fix_ts=fixed_ts,
                  ...
                  exploration_initial_eps=exploration_initial_eps, exploration_final_eps=exploration_final_eps, training_fraction=training_fraction, verbose=verbose, tripinfo=tripinfo,
                  discount_factor=discount_factor, batch_size=batch_size, replay_buffer_size=replay_buffer_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tripinfo', action='store_true', help='Fix signal yes or no')
    ...
    parser.add_argument('--training_fraction', type=float, default=0.5, help='Fraction of training data')
    ...
    parser.add_argument('--discount_factor', type=float, default=None, help='Discount factor for the DQN algorithm')
    parser.add_argument('--batch_size', type=int, default=None, help='Batch size for the DQN algorithm')
    parser.add_argument('--replay_buffer_size', type=int, default=None, help='Replay buffer size for the DQN algorithm')
    ...
    main(result_folder, args.proportion_of_saturations, args.net_path, args.total_timesteps, args.model_path, args.num_of_episodes, args.seed, args.fix_seed, args.fix_ts,
         args.learning_rate, args.learning_starts, args.train_freq, args.target_update_interval, args.exploration_initial_eps, args.exploration_final_eps, args.training_fraction, args.verbose, args.tripinfo,
         args.discount_factor, args.batch_size, args.replay_buffer_size)
