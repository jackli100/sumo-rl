...
class Train:
    def __init__(self, output_folder, net_file, route_file, total_timesteps, trained_model=None, num_of_episodes=10, seed=10,
                 fix_seed=False, fix_ts=False, learning_rate=0.0001, learning_starts=0, train_freq=1, target_update_interval=2000,
                 exploration_initial_eps=0.05, exploration_final_eps=0.01, training_fraction=0.5, verbose=1, tripinfo=False,
                 discount_factor=None, batch_size=None, replay_buffer_size=None):
        self.tripinfo = tripinfo
        if self.tripinfo:
            self.tripinfo_paht = os.path.join(output_folder, "tripinfos.xml")
            self.tripinfo_cmd = f"--tripinfo {self.tripinfo_paht}"
        else:
            self.tripinfo_cmd = None
        self.output_folder = output_folder
    ...
    def print_hyperparameters(self):
        with open(os.path.join(self.output_folder, "hyperparameters.txt"), "w") as f:
            ...
            f.write(f"tripinfo: {self.tripinfo}\n")
            ...
            f.write(f"training_fraction: {self.training_fraction}\n")
            f.write(f"verbose: {self.verbose}\n")
            f.write(f"discount_factor: {self.discount_factor}\n")
            f.write(f"batch_size: {self.batch_size}\n")
            f.write(f"replay_buffer_size: {self.replay_buffer_size}\n")
    ...
    def train(self):
        ...
        if self.discount_factor is not None:
            model_kwargs['gamma'] = self.discount_factor
        if self.batch_size is not None:
            model_kwargs['batch_size'] = self.batch_size
        if self.replay_buffer_size is not None:
            model_kwargs['buffer_size'] = self.replay_buffer_size
...
class ShowResults:
    def __init__(self, result_folder, log_file_path, metric, file_prefix="dqn_conn0_ep"):
    ...
    def main(self):
        ...
        with open(self.log_file_path, 'w') as log_file:
            ...
            for i, file_path in enumerate(file_paths, start=1):
                ...
                log_file.write(f'Average {self.metric} for episode {i}: {avg_value:.3f}\n')
